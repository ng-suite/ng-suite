...
module.exports = function generateCodeBox(component, key, title, doc, iframe) {
  ...
  // const componentName = `NsDemo${camelCase(capitalizeFirstLetter(component))}${camelCase(capitalizeFirstLetter(key))}Component`;
  ...
  // output = output.replace(/{{nsGenerateCommand}}/g, `ng g ng-zorro-antd:${component}-${key} -p app --styleext='less' --name=`);
  return output;
};
